...
- **Migrations are not auto-applied on boot.** After adding a migration, run `alembic upgrade head` against every target DB, including the local `dev.db`. Latest revision: `0011` (operator-defined agents).
...
2. **Agents** — Two sources resolved by `app/api/routes/agents.py::_resolve_runnable`: **(a) built-in code-defined** in `app/agents/registry.py` (`pm`, `compliance`, `document`) and **(b) operator-defined via the GUI**, stored in the `agents` table. The DB version wins at run time when an operator has one with the same name; disabled custom agents fall through to the built-in. Tools stay code-defined for safety — the operator picks from `ALL_TOOLS` via a checkbox list in `/agents`; they cannot create new tools or change tiers from the GUI. PM (`app/agents/pm.py`) orchestrates: `check_intake_status` (T0) + `assign_task` (T1) creates queued sub-`Task`s for compliance/document; PM itself takes no external action.
