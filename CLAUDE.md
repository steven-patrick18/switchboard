...
- **Migrations are not auto-applied on boot.** After adding a migration, run `alembic upgrade head` against every target DB, including the local `dev.db`. Latest revision: `0012` (agent lessons).
