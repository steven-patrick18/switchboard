...
- **Migrations are not auto-applied on boot.** After adding a migration, run `alembic upgrade head` against every target DB, including the local `dev.db`. Latest revision: `0013` (per-filing applications). (Production: the Dockerfile entrypoint runs `alembic upgrade head` automatically on boot, so Railway deploys "just work".)
...
1. **Agent runtime + approval gateway** — `app/agents/base.py::run_agent` is a manual Claude tool-use loop. Tools (`app/agents/tools.py`, `Tool` dataclass) are tier-classified using constants in `app/models/approval.py`: **T0/T1 run inline; T2/T3 are intercepted → an `Approval` (decision=pending) is created and the action does NOT execute.** This is the core safety boundary — never bypass it. A tool is either pure (`runner(args)`) or workspace-aware (`db_runner(args, ToolContext)` — async, receives db/client_id/project_id). Each agent has a scoped tool whitelist; prompt caching on the frozen system prompt; adaptive thinking; per-run token/cost in `AgentRun`. Every code-defined tool is registered in `app/agents/tools.py::ALL_TOOLS` — that's the single source of truth the GUI catalog uses.
2. **Agents (six-role roster)** — Built-ins in `app/agents/registry.py`: `pm` (orchestrator), `intake` (client data + capture-once), `compliance` (federal FCC scope only), `state_licensing` (per-state CPCNs/PUC), `carrier` (wholesale interconnection + STIR/SHAKEN), `document` (drafts + signing). Operator can ALSO create custom agents through the GUI; these are resolved by `app/api/routes/agents.py::_resolve_runnable` and take precedence over a built-in of the same name at run time. Tools stay code-defined for safety — the operator picks from `ALL_TOOLS` via a checkbox list in `/agents`; they cannot create new tools or change tiers from the GUI. PM never takes external action — it only delegates via `assign_task`.
3. **Agents learn from operator corrections** — `app/agent_learning.py`. Every approval rejection (`reason`) and every edit-before-approve (`payload_override` + `note`) writes an `AgentLesson` row tagged with the agent that originally queued the action. At run time, `run_agent` fetches the operator's most recent lessons for that agent (`LESSON_INJECT_LIMIT = 30`) and prepends them to the system prompt as an uncached "PAST CORRECTIONS" block — supervised in-context learning, no fine-tuning. Owner-scoped: an operator's corrections never bleed into another operator's runs. Manual lessons can also be added via `POST /agents/{name}/lessons` and shown alongside the captured ones in the `/agents` lessons modal.
4. **Per-filing stage tracker (`Application`)** — `app/models/application.py` + `app/applications.py`. Every regulated launch has parallel applications (OCN, FCC 499, RMD, STIR/SHAKEN, Section 214, per-state CPCNs, per-carrier interconnects). `resolve_required_applications(intake)` derives the required set from the client's intake just like `resolve_required_documents` does for docs. `POST /clients/{id}/applications/sync` is idempotent — creates missing rows without touching existing ones, so re-syncing after adding a target_state picks up the new CPCN row only. Each row has a coarse stage (`not_started`, `in_progress`, `awaiting_approval`, `submitted`, `under_review`, `complete`, `blocked`) and a `current_agent` field. Agents update both via the `update_application_stage` tool (T1, available to intake/compliance/state_licensing/carrier), so the UI shows "compliance is working on FCC 499" in real time as the agent loop runs. The operator can manually override any field at any time.
