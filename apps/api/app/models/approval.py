...
class Approval(Base):
    ...
    # Outbound email packet (to / from / subject / body) generated by
    # the executor for filings that need to be mailed externally (NECA
    # OCN, state CPCNs, carrier LOAs). The operator can one-click send
    # if SMTP is configured, or copy-paste manually otherwise.
    email_packet: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    email_sent_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    # Outbound Message-ID; matched against inbound replies when the
    # IMAP/webhook integration lands. For now just recorded after send.
